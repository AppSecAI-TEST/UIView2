...
public class AitHelper {
    ...
    public static boolean isAitMessage(IMMessage message) {
        ...
        boolean isForce = option != null && option.isForcePush() &&
                (option.getForcePushList() == null || option.getForcePushList().contains(UserCache.instance().getUserInfoBean().getUsername()));
    ...
    private static boolean isContentAit(String content) {
        ...
        Pattern pattern = Pattern.compile("(@" + UserCache.getUserAccount() + " )");
    ...
    public static ImageSpan getInputAitSpan(String name, float textsize) {
        ...
        return new ImageSpan(ValleyApp.getApp().getApplicationContext(), bmp, ImageSpan.ALIGN_BOTTOM);
    }
    ...
    public static String getAitName(GroupMemberBean member) {
        ...
        return NimUserInfoCache.getInstance().getUserName(member.getUserId());
