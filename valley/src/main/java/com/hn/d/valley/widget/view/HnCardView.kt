 ...
 * Version: 1.0.0
 */
class HnCardView(context: Context, attributeSet: AttributeSet? = null) : View(context, attributeSet) {
    ...
    fun setTargetCards(targetCards : String) {
        if (TextUtils.isEmpty(targetCards)) {
            return
        }

        val split = targetCards.split(",".toRegex()).dropLastWhile({ it.isEmpty() }).toTypedArray()
        val pokers = IntArray(split.size)
        var index = 0
        for (value in split) {
            val subValue = value.substring(2)
            pokers[index++] = Integer.valueOf(subValue)
        }
        setTargetCards(pokers)
